...
namespace effects
{
  void pushleds(MiLed& miled, const Led& color, int num, int del)
  {
      int i = 0;
  ...
  void sendfadeleds(MiLed& miled,Led color,int del)
  {
    int num = 15;
  ...
  void pushledsrandom(MiLed& miled, int num, int del)
  {
      ...
      {
          for (int j = 0; j < num; j++)
              if(i-j>=0)miled[i - j] = Led(random(0,30),random(0,30),random(0,30));
          if (i >= num)miled[i-num] = Led(0);
  ...
  void gravity(MiLed& miled,const Led& color, int del)
  {
    for(int i = 0;i < miled.get_count();i++)
    {
      if(i>0)miled[i-1] = Led(0);
      miled[i] = color;
      miled.show();
      delay(del);
    }
    ...
    {
      if(i<miled.get_count()-1)miled[i+1] = Led(0);
      miled[i] = color;
      miled.show();
      delay(del);
    }
  ...
  void gravitypush(MiLed& miled,const Led& color, int del)
  {
  ...
  void firework(MiLed& miled,int del)
  {
    ...
    for(int n = 0; n<10;n++)
      for(int i = 0;i<miled.get_count()/2;i++)
      {
        if(random(0,2)==1)
        {
          miled[miled.get_count()/2+i] = Led(0);
          miled[miled.get_count()/2-i] = Led(0);
          miled.show();
        }
        else
        {
          miled[miled.get_count()/2+i] = Led(random(0,30),random(0,30),random(0,30));
          miled[miled.get_count()/2-i] = Led(random(0,30),random(0,30),random(0,30));
          miled.show();
          if(n == 0)
          delay(50);
